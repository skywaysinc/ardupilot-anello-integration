...
void AP_ExternalAHRS_AnelloEVK::build_packet()
{
    ...
    while (nbytes-- > 0) {
        ...
        case ParseState::Reading_Data:
            ...
            break;
        case ParseState::Checking_CRC:
            message_in.packet.CRC[message_in.index++] = b;
            if (message_in.index >= 2) {
                message_in.state = ParseState::WaitingFor_Preamble;
                message_in.index = 0;
...
bool AP_ExternalAHRS_AnelloEVK::valid_packet(const AnelloEVK_Packet & packet) const
{
    ...
    return packet.CRC[0] == checksum_one && packet.CRC[1] == checksum_two;
}
...
void AP_ExternalAHRS_AnelloEVK::handle_filter(const AnelloEVK_Packet &packet)
{
    ...
    for (uint8_t i = 0; i < packet.data_packet_length; i += packet.payload[i]) {
        ...
        case FilterPacketField::GPS_TIME: {
            ins_data.tow_ms = extract_double(packet.payload, i+2) * 1000; // Convert seconds to ms
            ins_data.week = be16toh_ptr(&packet.payload[i+10]);
            break;
        }
        // LLH Position
        case FilterPacketField::LLH_POSITION: {
            ins_data.lat = extract_double(packet.payload, i+2) * 1.0e7; // Decimal degrees to degrees
            ins_data.lon = extract_double(packet.payload, i+10) * 1.0e7;
            ins_data.hae_altitude = extract_double(packet.payload, i+26) * 1.0e2; // Meters to cm
            break;
        }
        // NED Velocity
        case FilterPacketField::NED_VELOCITY: {
            ins_data.ned_velocity_north = extract_float(packet.payload, i+2);
            ins_data.ned_velocity_east = extract_float(packet.payload, i+6);
            ins_data.ned_velocity_down = extract_float(packet.payload, i+10);
            break;
        }
        // Filter Status
...
void AP_ExternalAHRS_AnelloEVK::post_filter() const
{
    {
        WITH_SEMAPHORE(state.sem);
        state.velocity = Vector3f{ins_data.ned_velocity_north, ins_data.ned_velocity_east, ins_data.ned_velocity_down};
        state.have_velocity = true;

        state.location = Location{ins_data.lat, ins_data.lon, gnss_data.msl_altitude, Location::AltFrame::ABSOLUTE};
        state.have_location = true;
    }

    AP_ExternalAHRS::gps_data_message_t gps {
        gps_week: ins_data.week,
        ms_tow: ins_data.tow_ms,
        ...
        longitude: ins_data.lon,
        latitude: ins_data.lat,
        msl_altitude: gnss_data.msl_altitude,

        ned_vel_north: ins_data.ned_velocity_north,
        ned_vel_east: ins_data.ned_velocity_east,
        ned_vel_down: ins_data.ned_velocity_down,
    };

    if (gps.fix_type >= 3 && !state.have_origin) {
        WITH_SEMAPHORE(state.sem);
        state.origin = Location{int32_t(ins_data.lat),
                                int32_t(ins_data.lon),
